...
if($_REQUEST['act']=='index')
{
	...
	$news=index_get_new_articles(10);
	$shopCourse=index_get_new_articles(13);

	$smarty->assign('news',$news);
	$smarty->assign('courses',$shopCourse);
	IndexHTML($recordcount);
	
}
if($_REQUEST['act']=='fee_query')
{
    $weight = intval($_REQUEST['weight']);

    $smarty->assign('weight',$weight);
    $smarty->assign('pageTitle',"运费计算器");
    ...
    $smarty->assign('allShipment',json_encode($allShipment));




    $smarty->display('fee_query.htm');
}
/*会员中心*/
elseif($_REQUEST['act']=='member_account')
{
	if(isset($_SESSION['username']))
	{
		...
		$allWareHouse = engrave_warehouse_list();
		//包裹提醒
		$package = engrave_package_remind_submit($user_id);
		$smarty->assign('package',$package);
		$smarty->assign('allWareHouse',$allWareHouse['warehouse_list']);
	 	//订单提醒
...
elseif($_REQUEST['act']=='list_haitao')
{
	//文章
	$recordcount=$_LANG['record_count'];
	$article=engrave_article_list(0,0,0,'',1,$recordcount);
	...
	$smarty->assign('transObj',$trans['channel']);
	$smarty->assign('username',$username);
	$smarty->display('list_haitao.htm');
}
/*FAQ2*/
elseif($_REQUEST['act']=='list_faq')
{
	...
	$categoryid = 8;//isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
	if($categoryid==0)
	{
...
//转动必读 cfang created for 放置 海淘教程, 禁运列表, 费用时效 等
elseif($_REQUEST['act']=='list_bidu')
{
    //文章
    $recordcount=20;
    ...
    $categoryid = 7;//isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
    if($categoryid==0)
    {
        $faq=engrave_channel('',8);
    }
...
function index_get_new_articles($catId)
{
    $sql = 'SELECT * ' .
            ' FROM ' . $GLOBALS['engrave']->table('article') .
            ' WHERE CntCategoryId= '.$catId.' and CntAuditingStatus=2   ' .
            ' ORDER BY CntTime DESC LIMIT 0,7';
    $res = $GLOBALS['engrave_db']->getAll($sql);
    foreach ($res AS $idx => $row)
    {
        $res[$idx]['id']          = $row['CntId'];

        $res[$idx]['add_time']    = date("Y-m-d", strtotime($row['CntTime']));
