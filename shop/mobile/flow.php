    ...
    else
    {
        ...
        if (!empty($_POST['act']) && $_POST['act'] == 'signin')
        {
            ...
            if (($captcha & CAPTCHA_LOGIN) && (!($captcha & CAPTCHA_LOGIN_FAIL) || (($captcha & CAPTCHA_LOGIN_FAIL) && $_SESSION['login_fail'] > 2)) && gd_version() > 0)
            {
                ...
                {
                    show_message_wap($_LANG['invalid_captcha']);
                }
            ...
            {
                $_SESSION['login_fail']++;
                show_message_wap($_LANG['signin_failed'], '', 'flow.php?step=login');
            }
        }
...
elseif ($_REQUEST['step'] == 'consignee')
{
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'GET')
    {
        ...
        $smarty->assign('real_goods_count', exist_real_goods(0, $flow_type) ? 1 : 0);
				...
				$sql = "SELECT us.*,u.address_id as default_id FROM " . $GLOBALS['ecs']->table('users') . " u" ." LEFT JOIN " . $GLOBALS['ecs']->table('user_address') . " us" . " ON u.user_id = us.user_id WHERE us.city <> '' AND u.user_id = '$_SESSION[user_id]' limit 5";
    ...
    else
    {
        ...
        $consignee = array(
            ...
            'best_time'     => empty($_POST['best_time'])  ? '' :   compile_str($_POST['best_time']),
				...
				$isinput = empty($_POST['isinput']) ? 0  :   intval($_POST['isinput']);
        ...
        ecs_header("Location: flow.php?step=checkout&isinput=$isinput\n");
        exit;
    }
}
elseif ($_REQUEST['step'] == 'drop_consignee')
{
    ...
    {
        show_message_wap($_LANG['not_fount_consignee']);
    }
}
elseif ($_REQUEST['step'] == 'checkout')
{
    ...
    if (empty($_SESSION['direct_shopping']) && $_SESSION['user_id'] == 0)
    {
		...
		// YMB MOD
		if($_REQUEST['isinput'] == 0 && !empty($_POST['address_id'])){
    ...
    /* 对商品信息赋值 */
    $cart_goods = cart_goods_list($flow_type); // 取得商品列表，计算合计
...
elseif ($_REQUEST['step'] == 'done')
{
    ...
    if (empty($cart_goods))
    {
        show_message_wap($_LANG['no_goods_in_cart'], $_LANG['back_home'], './', 'warning');
    }

    /* 检查商品总额是否达到最低限购金额 */
    if ($flow_type == CART_GENERAL_GOODS && cart_amount(true, CART_GENERAL_GOODS) < $_CFG['min_goods_amount'])
    {
        show_message_wap(sprintf($_LANG['goods_amount_not_enough'], price_format($_CFG['min_goods_amount'], false)));
    }
   ...
   /* 判断是不是实体商品 */
    ...
    if(isset($is_real_good))
    {
        ...
        if(!$db->getOne($sql))
        {
           show_message_wap($_LANG['flow_no_shipping']);
        }
    }
    /* 订单中的总额 */
    $total = order_fee($order, $cart_goods, $consignee);
...
elseif ($_REQUEST['step'] == 'update_cart')
{
    ...
    show_message_wap($_LANG['update_cart_notice'], $_LANG['back_to_cart'], 'flow.php');
    exit;
}
...
elseif ($_REQUEST['step'] == 'add_favourable')
{
    ...
    if (empty($favourable))
    {
        show_message_wap($_LANG['favourable_not_exist']);
    }

    /* 判断用户能否享受该优惠 */
    if (!favourable_available($favourable))
    {
        show_message_wap($_LANG['favourable_not_available']);
    }
    ...
    {
        show_message_wap($_LANG['favourable_used']);
    }
    ...
    if ($favourable['act_type'] == FAT_GOODS)
    {
        ...
        if (empty($_POST['gift']))
        {
            show_message_wap($_LANG['pls_select_gift']);
        }
        ...
        if (!empty($gift_name))
        {
            show_message_wap(sprintf($_LANG['gift_in_cart'], join(',', $gift_name)));
        }
        ...
        {
            show_message_wap($_LANG['gift_count_exceed']);
        }
