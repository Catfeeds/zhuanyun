...
if(!defined('XH_PARAM_INT'))
{
	define("XH_PARAM_INT",0);
}
if(!defined('XH_PARAM_TXT'))
{
	define("XH_PARAM_TXT",1);
}
/**
 * 获取安全参数
 * @param unknown $pi_strName：参数名称
 * @param string $pi_Def：若获取不到参数，则返回此值
 * @param string $pi_iType：类型-int；text
 * @return string|mixed：返回安全参数
 */
...
function engrave_load_config()
{
	...
	if ($data === false)
	{
		...
		$arr['s_waybillformat']  =!empty($arr['s_waybillformat']) ? trim($arr['s_waybillformat']) :'{Time}{Number}';
		$arr['s_waybilltimeformat'] =!empty($arr['s_waybilltimeformat']) ?  trim($arr['s_waybilltimeformat']) : 'ymd';
		$arr['s_orderwaterlevel'] = !empty($arr['s_orderwaterlevel']) && intval($arr['s_orderwaterlevel']) > 0 ? intval($arr['s_orderwaterlevel']) : 3;
		$arr['s_weightunit'] = !empty($arr['s_weightunit']) ? trim($arr['s_weightunit']) : 'LBS';
		$arr['s_waybillwaterlength'] = !empty($arr['s_waybillwaterlength']) && intval($arr['s_waybillwaterlength']) > 0 ? intval($arr['s_waybillwaterlength']) : 6;
		$arr['promote_number'] = !empty($arr['promote_number']) && intval($arr['promote_number']) > 0 ? intval($arr['promote_number'])  : 3;
...
function pagePrivCheck($priv) {
	$is_ajax = intval($_REQUEST['is_ajax']) ? 1 : 0;
	$is_ajax && check_authz_json($priv);
	!$is_ajax &&  admin_priv($priv);
}