...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    /* 插件的作者 */
    $modules[$i]['author']  = 'UTSOFT R&D TEAM';

    /* 插件作者的官方网站 */
    $modules[$i]['website'] = 'http://qdutsoft.com/';
...
class ucenter extends integrate
{
    ...
    function remove_user($id)
    {
        ...
        if ($col)
        {
            ...
            $GLOBALS['db']->query($sql);
            $sql = "UPDATE " . $GLOBALS['ecs']->table('users') . " SET isdelete=1 WHERE " . db_create_in($col, 'user_id'); //删除用户
            $GLOBALS['db']->query($sql);
            /* 删除用户订单 */
...
//             $GLOBALS['db']->query($sql);
        ...
        $sql = "update " . $GLOBALS['ecs']->table('users') . " set isdelete=1 WHERE ";
        if (is_array($post_id))
        {
