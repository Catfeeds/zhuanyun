...
<table width="100%" >
  ...
  <tr>
    ...
    <tr>
    <td class="label">入库码:</td>
    <td><input type="text" name="storage_code" maxlength="60" size="40" value="<?php echo $this->_var['user']['storage_code']; ?>" /><?php echo $this->_var['lang']['require_field']; ?></td>
  </tr>
      <tr>
    <td class="label">是否验证:</td>
    <td>
    <select name="is_validated">
      <option value="0" <?php if ($this->_var['user']['is_validated'] == 0): ?>selected='selected'<?php endif; ?>>未验证</option>
      <option value="1" <?php if ($this->_var['user']['is_validated'] == 1): ?>selected='selected'<?php endif; ?>>验证</option>
    </select>
    </td>
  </tr>
  <tr>
    <td class="label">订单支付方式:</td>
    <td>
    <label><input type=radio name="user_payment_type" value="正常"
      <?php if ($this->_var['user']['user_payment_type'] == "正常" || ! $this->_var['user']['user_payment_type']): ?>checked<?php endif; ?>>正常</label>
    <label><input type=radio name="user_payment_type" value="月结" 
      <?php if ($this->_var['user']['user_payment_type'] == "月结"): ?> checked <?php endif; ?>>月结</label>
    </td>
  </tr>
