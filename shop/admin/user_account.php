...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    $sql = "SELECT PaymentId, PaymentName FROM ".$engrave->table('payment').
           " WHERE IsDeletePayment = 0  ORDER BY PaymentId";
    
    $res = $db->query($sql);

    while ($row = $db->fetchRow($res))
    {
        $payment[$row['PaymentId']] = $row['PaymentName'];
    }
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($_REQUEST['act'] == 'insert')
    {
        ...
        $sql = "INSERT INTO " .$ecs->table('user_account').
               "(user_id,admin_user,amount,add_time,paid_time,admin_note,user_note,process_type,payment,is_paid)  VALUES ('$user_id', '$_SESSION[admin_name]', '$amount', '".gmtime()."', '".gmtime()."', '$admin_note', '$user_note', '$process_type', '$payment', '$is_paid')";
        $db->query($sql);
        $id = $db->insert_id();
    }
    else
...
function account_list()
{
    ...
    if ($result === false)
    {
        ...
        $sql = "SELECT COUNT(*) FROM " .$GLOBALS['ecs']->table('user_account'). " AS ua ".
                   $where;//$GLOBALS['ecs']->table('users') . " AS u " . 
        ...
        /* 查询数据 */
        $sql  = 'SELECT ua.*, u.user_name as member_user_name,PaymentName,adminuser.user_name FROM ' .
            $GLOBALS['ecs']->table('user_account'). ' AS ua LEFT JOIN ' .
            $GLOBALS['ecs']->table('users'). ' AS u ON ua.user_id = u.user_id'.
            ' left join '.
            $GLOBALS['engrave']->table('payment').' on payment = PaymentId '.
            ' left join '.
            $GLOBALS['ecs']->table('admin_user').' as adminuser on adminuser.user_id = admin_user '.
