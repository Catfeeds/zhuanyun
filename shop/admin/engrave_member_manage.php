...
/* 载入语言文件 */
require_once(ROOT_PATH . 'languages/' .$_CFG['lang']. '/admin/engrave_member_manage.php');
/*载入-会员管理-数据访问文件*/
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $ur_here = $_LANG['06_member_manage'] .'｜'.$_LANG['0605_member_manage'];
    $smarty->assign('ur_here', $ur_here);
    $ur_tip = empty($_LANG['engrave_member_manage_tip'])?'':$_LANG['engrave_member_manage_tip'];
    $smarty->assign('ur_tip', $ur_tip);
    $smarty->assign('action_link',  array('text' => $_LANG['0606_add_member'], 'href'=>'engrave_member_manage.php?act=add'));

    $smarty->assign('full_page',    1);

    $member_list= engrave_member_list();
    $smarty->assign('member_list',   $member_list['member_list']);
    ...
    /* 显示购物指南页面 */
    assign_query_info();
    $smarty->display('engrave_member_list.htm');
}
elseif($_REQUEST['act'] == 'add')
{
    ...
    $ur_here = $_LANG['06_member_manage'] .'｜'.$_LANG['0606_add_member'];
    $smarty->assign('ur_here', $ur_here);
    $smarty->assign('action_link',  array('text' => $_LANG['0605_member_manage'], 'href'=>'engrave_member_manage.php?act=list'));
    $smarty->assign('form_action', 'insert');

    /* 显示购物指南页面 */
    assign_query_info();
    $smarty->display('engrave_member_add.htm');
}
