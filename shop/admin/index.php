...
elseif ($_REQUEST['act'] == 'menu')
{
    ...
    //////////////////////////////////////////////////////////////////////////////////////////////////
    //左侧菜单实现方式：
    //菜单栏：inc_menu.php中配置好对应名称
    //菜单权限：inc_priv.php中配置好对应名称、功能
    //语言栏common.php中配置好对应名称
    //////////////////////////////////////////////////////////////////////////////////////////////////    
    //////////////////////////////////商城：左侧菜单///////////////////////////////
    ...
    //////////////////////////////////商城：左侧菜单///////////////////////////////
    //////////////////////////////////物流转运：左侧菜单///////////////////////////////
    foreach ($engrave_modules AS $key => $value)
    {
    	ksort($engrave_modules[$key]);
    }
    ksort($engrave_modules);
	
    foreach ($engrave_modules AS $key => $val)
    {
    	$engrave_menus[$key]['label'] = $_LANG[$key];
    	if (is_array($val))
    	{
    		foreach ($val AS $k => $v)
    		{
    			...
    			$engrave_menus[$key]['children'][$k]['label']  = $_LANG[$k];
    			$engrave_menus[$key]['children'][$k]['action'] = $v;
    		}
    	}
    	else
    	{
    		$engrave_menus[$key]['action'] = $val;
    	}
    	// 如果children的子元素长度为0则删除该组
    	if(empty($engrave_menus[$key]['children']))
    	{
    		unset($engrave_menus[$key]);
    	}
    }
    //////////////////////////////////物流转运：左侧菜单///////////////////////////////

    //////////////////////////////////物流转运：左侧会员菜单///////////////////////////////
    foreach ($engrave_member_modules AS $key => $value)
    {
    	ksort($engrave_member_modules[$key]);
    }
    ksort($engrave_member_modules);
    
    foreach ($engrave_member_modules AS $key => $val)
    {
    	$engrave_member_menus[$key]['label'] = $_LANG[$key];
    	if (is_array($val))
    	{
    		foreach ($val AS $k => $v)
    		{
    			...
    			$engrave_member_menus[$key]['children'][$k]['label']  = $_LANG[$k];
    			$engrave_member_menus[$key]['children'][$k]['action'] = $v;
    		}
    	}
    	else
    	{
    		$engrave_member_menus[$key]['action'] = $val;
    	}
    	// 如果children的子元素长度为0则删除该组
    	if(empty($engrave_member_menus[$key]['children']))
    	{
    		unset($engrave_member_menus[$key]);
    	}
    }
    //////////////////////////////////物流转运：左侧会员菜单///////////////////////////////
...
elseif ($_REQUEST['act']=='engrave_main')
{
	$smarty->display('engrave_start.htm');
}
...
//    $modules = read_modules('../includes/modules/shipping');
    ...
    $modules     = array();
    //物流转运
    $engrave_modules=array();
    $engrave_member_modules=array();
