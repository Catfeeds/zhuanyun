 ...
 * @return  array
 */
function engrave_group_list($real_goods=1, $conditions = '')
{
	/* 记录总数 */
	$sql_count = "SELECT COUNT(*) FROM " .$GLOBALS['engrave']->table('adminrole');
	$filter['record_count'] = $GLOBALS['engrave_db']->getOne($sql_count);
	/* 分页大小 */
	$filter = page_and_size($filter);

	$sql = "SELECT RankId,TypeName " .
			" FROM " . $GLOBALS['engrave']->table('adminrole') .
