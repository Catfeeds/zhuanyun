 ...
 * 载入列表
 */
listTable.loadList = function(cb)
{
  var args = "act="+this.query+"" + this.compileFilter();
  if(cb) { this.loadListCb = cb;}
  Ajax.call(this.url, args, this.listCallback, "POST", "JSON");
}

listTable.loadExport = function(act)
{
    var params = "act="+act+"" + this.compileFilter();
    var url = this.url;
    var d = new Date();
 ...
 * 移除返回
 */
listTable.remove_return = function(id,cfm,opt)
{
  if (opt == null)
  {
    opt = "remove_return";
 ...
 */
listTable.removegroup = function(id, groupid, cfm, opt)
{
  if (opt == null)
  {
    opt = "removegroup";
  ...
  if (confirm(cfm))
  {
    var args = "act=" + opt + "&id=" + id + "&groupid=" + groupid + this.compileFilter();
...
/**
 * 删除服务设置中的一个记录----修改服务设置名称 添加--2015年1月11日 23:09:30
 */
listTable.removesetting = function(id, serviceid, cfm, opt)
{
  if (opt == null)
  {
    opt = "removesetting";
  ...
  if (confirm(cfm))
  {
	  var args = "act=" + opt + "&id=" + id + "&serviceid=" + serviceid + this.compileFilter();   
      Ajax.call(this.url, args, this.listCallback, "GET", "JSON");
  }
}
...
listTable.listCallback = function(result, txt)
{
  ...
  else
  {
    try
    {
      ...
      if(typeof listTable.loadListCb == 'function') {
        listTable.loadListCb(result);
      }
      //_xml_htmlInsert(document.getElementById('listDiv'),result.content);
