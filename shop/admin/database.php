...
if ($_REQUEST['act'] == 'backup')
{
    ...
    $engrave_tables = $engrave_db->GetCol("SHOW TABLES LIKE '" . mysql_like_quote($engrave->prefix) . "%'");
   
//     foreach ($engrave_tables as $key=>$val)
//     {
//     	echo $key.'=>'.$val.'<br>';
//     }
    ...
    $smarty->assign('engrave_tables',$engrave_tables);
    $smarty->assign('vol_size', $allow_max_size);
    $name_time=cls_sql_dump::get_radom_name_time();
    $smarty->assign('shop_sql_name', $name_time . 'shop.sql');
    $smarty->assign('engrave_sql_name', $name_time . 'engrave.sql');
...
elseif ($_REQUEST['act'] == 'restore')
{
    ...
    $path = ROOT_PATH . DATA_DIR . '/sqldata/';
    $engrave_list = array();
    $engrave_path = ROOT_PATH . DATA_DIR . '/engravesqldata/';
    ...
    $engrave_mask = file_mode_info($engrave_path);
    if ($engrave_mask === false)
    {
    	$warning = sprintf($_LANG['dir_not_exist'], $engrave_path);
    	$smarty->assign('warning', $warning);
    }
    elseif (($engrave_mask & 1) < 1)
    {
    	$warning = $engrave_path . '&nbsp;&nbsp;' . $_LANG['cannot_read'];
    	$smarty->assign('warning', $warning);
    }
    else
    {
    	...
    	foreach ($real_list AS $file)
    	{
    		...
    		$file_size = filesize($engrave_path . $file);
    		$info = cls_sql_dump::get_head($engrave_path . $file);
    		$engrave_list[] = array('name' => $file, 'ver' => $info['ecs_ver'], 'add_time' => $info['date'], 'vol' => $info['vol'], 'file_size' => num_bitunit($file_size), 'mark' => $mark);
    	}
    }
    ...
    $smarty->assign('engrave_list',    $engrave_list);
...
elseif ($_REQUEST['act'] == 'dumpsql')
{
    ...
    {
        $sql_file_name = $dump->get_radom_name_time();
    }
    else
    {
    	// 过滤 0xa 非法字符
 ...
 * 转运
 */
elseif ($_REQUEST['act'] == 'engrave_dumpsql')
{
	/* 权限判断 */
	$token=trim($_REQUEST['token']);
	if($token!=$_CFG['engreave_token'])
	{
		sys_msg($_LANG['backup_failure'], 1);
	}
	admin_priv('db_backup');
	...
	/* 检查目录权限 */
	$path = ROOT_PATH . DATA_DIR . '/engravesqldata';
	$mask = file_mode_info($path);
	if ($mask === false)
	{
		$warning = sprintf($_LANG['dir_not_exist'], $path);
	...
	/* 初始化 */
	$dump = new cls_sql_dump($engrave_db);
	$run_log = ROOT_PATH . DATA_DIR . '/engravesqldata/run.log';

	/* 初始化输入变量 */
	if (empty($_REQUEST['engrave_sql_file_name']))
	{
		$engrave_sql_file_name = $dump->get_radom_name_time();
	}
	else
	{
		// 过滤 0xa 非法字符
		$engrave_sql_file_name = str_replace("0xa", '', trim($_REQUEST['engrave_sql_file_name']));
		$pos = strpos($engrave_sql_file_name, '.sql');
		if ($pos !== false)
		{
			$engrave_sql_file_name = substr($engrave_sql_file_name, 0, $pos);
		}
	}
	...
	{
		case 'full':
			$except = array($engrave->prefix.'sessions', $engrave->prefix.'sessions_data');
			$temp = $engrave_db->GetCol("SHOW TABLES LIKE '" . mysql_like_quote($engrave->prefix) . "%'");
			foreach ($temp AS $table)
			{
				if (in_array($table, $except))
				{
		...
		case 'stand':
			$temp = array('area','article','channel','collogistics',
			'complaint','coupon','currecy','deliveryaddress',
			'enum','enumgroup','evalua','faq',
			'focusmap','moduleservice','order','order_attachs',
			'order_log','ordergoods','orderservice','orderwaybill',
			'package','package_attachs','package_service','package_shoppingvouchers',
			'package_track','packagegoods','payment','paymentinterface',
			'prepaid','ratetax_table','services','services_setting',
			'servicesgroup','shipping','shipping_declaredvalue','shipping_ladderprice',
			'shipping_ladderprice_group','siteclassification',
			'sitemanage','stationinformation',
			'statucode','system_config',
			'template','warehouse','waybillservice');
			foreach ($temp AS $table)
			{
				$tables[$engrave->prefix . $table] = -1;
			}
			$dump->put_tables_list($run_log, $tables);
			break;

		case 'min':
			$temp = array('collogistics',
					'complaint','coupon','currecy','deliveryaddress',
					'focusmap','moduleservice','order','order_attachs',
					'order_log','ordergoods','orderservice','orderwaybill',
					'package','package_attachs','package_service','package_shoppingvouchers',
	...
	if (empty($tables))
	{
		...
		{
			/* 有多个文件 */
			if (!@file_put_contents(ROOT_PATH . DATA_DIR . '/engravesqldata/' . $engrave_sql_file_name . '_' . $vol . '.sql', $dump->dump_sql))
			{
				sys_msg(sprintf($_LANG['fail_write_file'], $engrave_sql_file_name . '_' . $vol . '.sql'), 1, array(array('text'=>$_LANG['02_db_manage'], 'href'=>'database.php?act=backup')), false);
			}
			$list = array();
			for ($i = 1; $i <= $vol; $i++)
			{
			$list[] = array('name'=>$engrave_sql_file_name . '_' . $i . '.sql', 'href'=>'../' . DATA_DIR . '/engravesqldata/' . $engrave_sql_file_name . '_' . $i . '.sql');
			}
		...
		{
			/* 只有一个文件 */
			if (!@file_put_contents(ROOT_PATH . DATA_DIR . '/engravesqldata/' . $engrave_sql_file_name . '.sql', $dump->dump_sql))
			{
				sys_msg(sprintf($_LANG['fail_write_file'], $engrave_sql_file_name . '_' . $vol . '.sql'), 1, array(array('text'=>$_LANG['02_db_manage'], 'href'=>'database.php?act=backup')), false);
			};

			$smarty->assign('list',  array(array('name' => $engrave_sql_file_name . '.sql', 'href' => '../' . DATA_DIR . '/engravesqldata/' . $engrave_sql_file_name . '.sql')));
			$smarty->assign('title', $_LANG['backup_success']);
			$smarty->display('sql_dump_msg.htm');
		}
	}
	else
	{
		...
		$lnk = 'database.php?act=dumpsql&token='.$_CFG['token'].'&engrave_sql_file_name=' . $engrave_sql_file_name . '&vol_size=' . $max_size . '&vol=' . ($vol+1);
        $smarty->assign('title',         sprintf($_LANG['backup_title'], '#' . $vol));
        		$smarty->assign('auto_redirect', 1);
        		$smarty->assign('auto_link',     $lnk);
		$smarty->display('sql_dump_msg.htm');
 ...
 * 删除备份 - 转运
 */
elseif ($_REQUEST['act'] == 'engrave_remove')
{
    /* 权限判断 */
    admin_priv('db_backup');
 ...
 * 从服务器上导入数据：转运
 */
elseif ($_REQUEST['act'] == 'engrave_import')
{
	/* 权限判断 */
	admin_priv('db_renew');
	...
	if (preg_match('/_[0-9]+\.sql$/', $file_name))
	{
		...
		{
			/* 提示用户要求确认 */
			sys_msg($_LANG['confirm_import'], 1, array(array('text'=>$_LANG['also_continue'], 'href'=>'database.php?act=engrave_import&confirm=1&file_name=' . $file_name)), false);
		}
		...
		foreach ($post_list AS $file)
		{
			...
			if (!engrave_sql_import($path . $file))
			{
				sys_msg($_LANG['sqlfile_error'], 1);
			}
		}
	...
	else
	{
		...
		if (engrave_sql_import($path . $file_name))
		{
			clear_cache_files();
			admin_log($_LANG['backup_time'] . $info['date'],'restore', 'db_backup');
			sys_msg($_LANG['restore_success'], 0, array(array('text'=>$_LANG['restore'], 'href'=>'database.php?act=restore')));
...
//-- 上传sql 文件：商城
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'upload_sql')
{
    /* 权限判断 */
    admin_priv('db_renew');
...
//-- 上传sql 文件：转运
/*------------------------------------------------------ */
elseif ($_REQUEST['act'] == 'engrave_upload_sql')
{
	/* 权限判断 */
	admin_priv('db_renew');
	...
	if (engrave_sql_import($sql_file))
	{
		clear_all_files();
		@unlink($sql_file);
		sys_msg($_LANG['restore_success'], 0, array());
...
elseif ($_REQUEST['act'] == 'optimize')
{
    ...
    $ret = $engrave_db ->query("SHOW TABLE STATUS LIKE '" . mysql_like_quote($engrave->prefix) . "%'");
    while ($row = $db->fetchRow($ret))
    {
    	if (strpos($row['Name'], '_session') !== false)
    	{
 ...
 * 数据表优化
 */
elseif ($_REQUEST['act'] == 'run_optimize')
{
    admin_priv('db_backup');
    $tables = $db->getCol("SHOW TABLES LIKE '" . mysql_like_quote($ecs->prefix) . "%'");
    ...
    $tables = $engrave_db->getCol("SHOW TABLES LIKE '" . mysql_like_quote($engrave->prefix) . "%'");
    foreach ($tables AS $table)
    {
        if ($row = $engrave_db->getRow('OPTIMIZE TABLE ' . $table))
        {
            /* 优化出错，尝试修复 */
            if ($row['Msg_type'] =='error' && strpos($row['Msg_text'], 'repair') !== false)
            {
                $engrave_db->query('REPAIR TABLE ' . $table);
            }
        }
    }
 ...
 * @return void
 */
function engrave_sql_import($sql_file)
{
	$db_ver  = $GLOBALS['engrave_db']->version();
	...
	if ($db_ver > '4.1')
	{
		for($i = 0; $i < $ret_count; $i++)
		{
			...
			if (!empty($ret[$i]))
			{
				...
				$GLOBALS['engrave_db']->query($ret[$i]);
			}
		}
	}
	else
