...
require_once (ROOT_PATH . 'admin/includes/finance.php');
$finance = new Finance();
$allCountries = allCountries();

$smarty->assign('lang',$_LANG);
$smarty->assign('allCountries',$allCountries);
 $smarty->assign('allShipments',   shipment_all_data_for_order_list());
 //默认货币的名称
...
if ($_REQUEST['act'] == 'list')
{
    ...
    //获得该订单的仓库名称
    //所有仓库列表 
    $allHouses = engrave_order_waybill_list();
    $smarty->assign('waybill_list', $allHouses);
    $smarty->assign('allhouses_list', $allHouses);
    
    $smarty->assign('ur_totalweight', $ur_totalweight);
    $smarty->assign('allorder_totalweight', $ur_totalweight);
    
    
    //获得重量的单位 系统中默认的
    $smarty->assign('weight_unit', $_EFG['s_weightunit']);
    //获得系统中默认的货币名称
    $smarty->assign('default_name', $currency_name);
    //获取订单管理列表---数据
...
elseif($_REQUEST['act'] == 'pack')
{
	...
	$orderlog['ol_userid'] =  $admin_id;
	$orderlog['ol_username'] = $admin_username;
	$orderlog['ol_info']='订单商品已由 '.$adminorder_view_username.
	' 送至打包';
...
// 	}
    ...
    //    	欧洲专线
	if($order['order_shipmentid'] == 2) {
        $dispatchInfo = new DispatchInfo();
        $dispatchId = $order['dispatch_id'];
        if($dispatchId) {
            $dispatchDetail = $dispatchInfo->getInfo($dispatchId);
                $dispatchDetail['dispatch_arrival_time'] = date( 'Y-m-d H:i',$dispatchDetail['arrival_time']);
            $smarty->assign('dispatchDetail', $dispatchDetail);
            $smarty->assign('dispatchId', $dispatchId);
        }

    }
	$listUrl = "engrave_all_orders.php?act=list";

	if(empty($order['order_id'])) {
        ecs_header("Location: $listUrl\n");
    }
	...
	foreach ($orderwaybill_list as $key => $val)
	{
        ...
        $orderwaybill_list[$key]['ordw_sendtime']  = date('Y-m-d H:i', $orderwaybill_list[$key]['ordw_sendtime']);
		//get ordergoods
        $sql = "SELECT goods.*, tax.rate_name FROM " . $GLOBALS['engrave']->table('ordergoods')." goods "
                ." JOIN " . $GLOBALS['engrave']->table('ratetax_table') ." tax"
            ." on tax.rate_id = goods.ordg_goodtype "
            ." WHERE ordg_waybillid='".$val['ordw_waybillid']."'";
        $order_goods_list = $GLOBALS['engrave_db']->getAll($sql);
        $orderwaybill_list[$key]['order_goods_list'] = $order_goods_list;
		$index++;
	}
	...
	$allShipment = shipment_all_data_for_order_list(true);


	$smarty->assign('shipment_list', $allShipment);
	//获得第三方物流
...
elseif($_REQUEST['act'] == 'waybill_delivery')
{
	...
	$waybillno = !empty($_REQUEST['waybillno']) ? trim($_REQUEST['waybillno']) : '';//运单号
	$sendtime = $_REQUEST['send_time'];
	if(!$sendtime) {
...
elseif($_REQUEST['act'] == 'order_delivery')
{
	...
	if(engrave_isexist_orderwaybillno($OrderId))
	{
	    //在表engrave_orderwaybill 中不存在没有ordw_waybillno 的记录
		if (!engrave_delivertstatus_trans("5", $OrderId,$conn))
		{
			transaction_failed($conn);
			make_json_error('修改发货状态为5,发货失败');
			return;
		}
	...
	else 
	{
		...
		{
			transaction_failed($conn);
			make_json_error('修改发货状态为51, 发货失败');
			return;
		}
		if(!engrave_package_update_trans("51", $OrderId,$conn))
		{
			//配货中
			transaction_failed($conn);
			make_json_error('修改包裹状态为51,发货失败');
			return;
		}
	}
	/*日志*/
...
elseif($_REQUEST['act'] == 'weight')
{
	...
	//获取订单管理列表---数据
	$order=engrave_order_byid($OrderId);
	if($order['order_shipment_oilDiscount']>0 && !$order['order_shipment_oilPrice']) {
        $order['order_shipment_oilPrice'] = $order['order_waybillcost'] * $order['order_shipment_oilDiscount']/100;
    }
	...
	//获得该订单的相关线路
	$smarty->assign('shipping_list', engrave_order_shipping_list());
	$allShipment = shipment_all_data_for_order_list();
	$smarty->assign('shipment_list', $allShipment);
	//获得货币名称
	$currency_name = engrave_currency_name();
	$smarty->assign('currency_name',$currency_name);
...
elseif($_REQUEST['act'] == 'order_insert')
{
	...
	$order_oilprice = !empty($_POST['oilprice']) ? doubleval($_POST['oilprice']) : 0.00;
	...
	$order_valueservicecost+$order_tariffcost+$order_oilprice;
...
// 	return;
	if($paymentpath == 2)
	{
		...
		$user_payment_type = $order['user_payment_type'];///'正常','月结'
		//消费金额和 货币兑换积分比例
		...
		$result = engrave_users_pay_money($totalmoneny,$user['user_money'],$user['user_jpymoney'],
				$user['user_subsidiarymoney'],$exchageRate,$pay_points,$user_id,$order_paymenttype,$conn,$user_payment_type);
		if($result===false)
		{
			//配货中
			transaction_failed($conn);
			make_json_error('日志[支付扣款failed]：称重打包失败');
			return;
		} else {
		    if(!engrave_income_insert($order, $totalmoneny,$user_payment_type, $conn)) {
                transaction_failed($conn);
                make_json_error('日志[income insert failed ]：称重打包失败');
                return;
            }
        }
		...
		$order_sql="update ".$GLOBALS['engrave']->table('order').
	         ...
	         "',order_shipment_oilPrice = '".$order_oilprice.
		...
		if($result===false)
		{
			//配货中
			transaction_failed($conn);
			make_json_error('日志5：称重打包失败');
			return;
		}  else {
		    //存入发票
            //$order
            $result = engrave_order_invoice_insert($conn, $order, $totalmoneny);
            if($result===false) {
                transaction_failed($conn);
                make_json_error('日志[invoice insert failed]：称重打包失败');
                return;
            }
        }
...
elseif($_REQUEST['act'] == 'getcurrentbalance')
{
	...
	echo $current_balance;//json_encode($current_balance);
}
//获得对应订单的包裹的信息
...
elseif($_REQUEST['act'] == 'print_picking')
{
	...
	if(count($order_ids)<=1)
	{
        ...
        //print_a($orderwaybill_list);
		//包裹内物品
		$package_goods_list = engrave_package_goods_list($OrderId);
		//增值服务
		$value_service_list = engrave_value_service_list($OrderId);

        $delivery_list = engrave_delivery_list($orderwaybill_list[0][ordw_consigid]);
...
elseif($_REQUEST['act'] == 'order_view')
{
	...
	$view['order_valueservicecost']+ $view['order_operatorcost']+
