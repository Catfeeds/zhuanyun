...

{
	...
	make_json_result($smarty->fetch('engrave_site_classification.htm'), '',
...

{
...

	{
		...
		$link[0]['href'] = 'engrave_addjisuan.php?act=add';

		$link[1]['text'] = $_LANG['back_class_list'];

		$link[1]['href'] = 'engrave_meun_manage.php?act=list';

		if(!engrave_mejisuan($_POST['fangshi']))
...

	{
		...
		$link[0]['href'] = 'engrave_jisuan_manage.php?act=list';
			...
			$sql="update" . $GLOBALS['engrave']->table('calculator').

"set gongshi='".$_POST['gongshi']."',neirong='".$_POST['neirong']."', paixu='".$_POST['paixu']."',fangshi='".$_POST['fangshi']."',huilv='".$_POST['huilv']."' where id=".$_REQUEST['ids'];

	return $GLOBALS['engrave_db']->query($sql);
			

			echo "<script>window.location.href='engrave_jisuan_manage.php?act=list';</script>";
