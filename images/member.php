...
function order_insert($submit_order,$engrave_db_host, $engrave_db_user, $engrave_db_pass, $engrave_db_name)
{
	...
	$submit_order['order_isdelivery']."','".$submit_order['order_iscolsed']."','".$submit_order['order_printstatus']."','".$submit_order['order_isdelete'].
	"','".$submit_order['order_paymenttype']."','".$submit_order['order_coupon']."',"."@identity".
	");";
	...
	for($i=0;$i<$waybill_number;$i++)
	{
		...
		")";
		mysql_query($sql,$conn);
		$result_waybill = mysql_query('select @identity as identity;',$conn);
		...
		for($j=0;$j<$goods_number;$j++)
		{
			...
			if($ordg_goodtype!=0)
			{
				...
				"(ordg_waybillid,ordg_goodtype,ordg_goodname,ordg_goodnumber,ordg_goodprice,ordg_delete) values".
			   	...
			   	$ordg_delete."')";
				$order_goods=mysql_query($sql,$conn);
 ...
 * @param unknown $conn
 */
function order_cancel_failed($conn)
{
	//添加失败
 ...
 * @param unknown $conn
 */
function order_cancel_success($conn)
{
	//添加失败
