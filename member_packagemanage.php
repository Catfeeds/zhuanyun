...
if(isset($_SESSION['username']))
{
	...
	if($users['user_isperfect'] == 0)  //session 中没有此字段
	{
		//获取国家
...
{
	$act=$_REQUEST['act']; //member_insert
	//获取所有页面信息
	...
	$forecast['pck_weight'] = isset($_REQUEST['pck_weight']) ? intval($_REQUEST['pck_weight']) : 0;
	//购物网站id
	$forecast['pck_ordersite'] = isset($_REQUEST['pck_ordersite']) ? intval($_REQUEST['pck_ordersite']) : "";
	//在网站上的订单号
	$forecast['pck_ordernumber'] = isset($_REQUEST['pck_ordernumber']) ? trim($_REQUEST['pck_ordernumber']) : "";
	$forecast['pck_sizelength'] = isset($_REQUEST['pck_sizelength']) ? intval($_REQUEST['pck_sizelength']) : 0;
	$forecast['pck_sizewidth'] = isset($_REQUEST['pck_sizewidth']) ? intval($_REQUEST['pck_sizewidth']) : 0;
	$forecast['pck_sizeheight'] = isset($_REQUEST['pck_sizeheight']) ? intval($_REQUEST['pck_sizeheight']) : 0;
...
elseif($_REQUEST['act']=="member_5304")
{
	...
	{
		$arr['list'][$key]['country_name'] = $area_countries[$val['da_country']];
	}
	echo json_encode($arr);
}
//==================库存列表==========================================================================
...
$sql="insert into ".$GLOBALS['engrave']->table('package').
		...
		.$forecast['pck_warehouseid'].","
		.$forecast['pck_expressid'].",'"
		.$forecast['pck_expressnumber']."','"
		.$forecast['pck_name']."','"
		.$forecast['pck_weight']."','"
		.$forecast['pck_ordersite']."','"
		.$forecast['pck_ordernumber']."','"
		.$forecast['pck_sizelength']."','"
		.$forecast['pck_sizewidth']."','"
		.$forecast['pck_sizeheight']."','"
		.$forecast['pck_userremark']."',"
		...
		if($result!==false)
		{
			$packageId = $GLOBALS['engrave_shop_db']->insert_id();
		}else 	{
			//添加失败
			submit_failed($conn);
			return;
	...
	for ($index = 0; $index < $pckgcount; $index++)
	{
		...
		$pckg_amount=isset($_REQUEST['pckg_amount'.$index]) ? intval($_REQUEST['pckg_amount'.$index]) : 1;
		$pckg_unitprice=isset($_REQUEST['pckg_unitprice'.$index]) ? doubleval($_REQUEST['pckg_unitprice'.$index]) : 0;
		$pckg_totalprice=$pckg_amount * $pckg_unitprice;//isset($_REQUEST['pckg_totalprice'.$index]) ? doubleval($_REQUEST['pckg_totalprice'.$index]) : 0;
		if($pckg_amount==0) {
			continue;
		}
		...
		$pck_goods = $engrave_db -> query($sql);
		if($pck_goods===false) {
			//添加失败
			submit_failed($conn);
			return;
	...
	foreach ($_FILES AS $code => $file)
	{
		...
		if($filename!='error')
		{
			...
			if($psv_vouchersvalue!=='')
			{
				...
				"(".$packageId.",'".$psv_vouchersvalue.
				"')";
				//$psvresult=mysql_query($sql,$conn);
				$psvresult = $engrave_db -> query($sql);
				if($psvresult===false)
				{
					//添加失败
					submit_failed($conn);
	...
	//插入 orderservice 表，不知道 这个SB 为什么要 生成RecordNo
	...
	$appreciation['HouseId'] = $forecast['pck_warehouseid'];
	$appreciation['UserId'] = $user_id;
	...
	$appreciation['ps_packageid'] = $packageId;
...
function member_stock()
{
	...
	$link[0]['href'] = 'member.php?act=package_forecast';
