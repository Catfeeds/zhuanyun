...
function getData(page){ 
	$.ajax({
		type: 'POST',
		url: 'member_packagemanage.php?act=member_3104',
		data: {'pageNum':(page-1),
			'StatusId':(tablename_index-1)},
		dataType:'json',
		beforeSend:function(){
...
function stock_callback(result)
{
	...
	if(result.length!=0)
	{
		...
		$.each(result,function(index,array){ //遍历json数据列
			...
			newTd0.innerHTML="<a href='member.php?act=member_service_detail&recordId="+array['RecordId']+"'>"+array['RecordNo']+"</a>";
			newTd1.innerHTML=array['HouseName'];
			newTd2.innerHTML=array['ServiceName'];
			newTd3.innerHTML=roundNumber(array['Price'],2);
			newTd4.innerHTML=array['Tel'];
			newTd5.innerHTML=array['StatusId']==1 ? "已处理":"未处理";
			if(array['StatusId']==0)
		    {
				operatorText = "<a href='member.php?act=member_service_detail&recordId="+array['RecordId']+"'>详细</a>&nbsp;&nbsp;<a href='member.php?act=member_service_delete&recordId="+array['RecordId']+"'>删除</a>";
		    }
			else
			{
				operatorText = "<a href='member.php?act=member_service_detail&recordId="+array['RecordId']+"'>详细</a>";
			}
			newTd6.innerHTML=operatorText;
		});
	}
	else
	{
