 ...
 */ (function ($) {
    ...
    $.smallslider = function (elm, options) {
        ...
        _this.init = function () {
            ...
            if (_this.itemNums > 1) {
                ...
                if (_this.opts.showAButtons) {
                    _this.createAButtons(); // 创建左右切换键。
                }
		...
		_this.createAButtons=function(){
			var aButtons='';
			aButtons+='<a href="javascript:;" class="sli_a sli_prev ie6png" target="_self"></a>';
			aButtons+='<a href="javascript:;" class="sli_a sli_next ie6png" target="_self"></a>';
            $(aButtons).appendTo(_this.$elm);
			_this.$abtns = _this.$elm.find('a.sli_a');
            _this.$abtns.click(function () {
				var ix = _this.$abtns.index($(this));
				if(ix==0){//向左
					if(_this.curNum==_this.itemNums-1){_this.curNum=-1;}
					_this.curNum++;
				}
				else{//向右
					if(_this.curNum==_this.itemNums-1){_this.curNum=-1;}
				...
				_this.slideTo(_this.curNum); // 表示需要切换到哪一张
			});
		};
    ...
    $.smallslider.defaults = {
        ...
        buttonPosition: 'none', // 按钮位置表示，共有四个值：leftTop，leftBottom，rightTop，rightBottom，none
		...
		showAButtons: true, //是否显示左右切换键，默认显示
