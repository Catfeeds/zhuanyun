...
function engrave_users($user_id=0)
{
	...
	"user_money,frozen_money,user_jpymoney,user_subsidiarymoney,pay_points,rank_points,address_id,reg_time,last_login,user_payment_type, ".
...
function engrave_order_ruku($orderno,$orderstatus,$consigname,$user_id = 0,$startPage=0)
{
   ...
   $where = "";
	...
	if($consigname != '')
	{
		$where = $where . (" and consigname like '%" . $consigname."%'");
	}
	$where = $where . " and order_userid='".$userid."'";
...
function member_register($user,$recommend_code='',$storage_code='',$conn)
{
	...
	"'$password_answer','$refencer_recommend_code','$ip','',0,0,0,'','','','','',0,".$user['user_warehouseid'].",'themes/default/images/default_avatar.png')";
...
function Login($username,$password)
{
	...
	"user_identitycard,user_country,user_province,user_city,user_address,user_zipcode,".
	"user_taobaowangwang,user_wechat,user_remark,user_isperfect, user_payment_type ".
	' from '. 
	$GLOBALS['engrave_shop']->table("users").
...
function get_user_info($username='', $email = '')
{
    ...
    $userInfo = $GLOBALS['engrave_shop_db']->getRow($sql);
    return $userInfo;
}
