 ...
 * $Id: lib_base.php 17217 2014年11月28日 11时03分00秒 zhangxingpeng $
 */
...
function send_mail($name, $email, $subject, $content, $type = 0, $notification=false)
{
    ...
    if ($GLOBALS['_SCFG']['mail_charset'] != EC_CHARSET)
    {
        $name      = ecs_iconv(EC_CHARSET, $GLOBALS['_SCFG']['mail_charset'], $name);
        $subject   = ecs_iconv(EC_CHARSET, $GLOBALS['_SCFG']['mail_charset'], $subject);
        $content   = ecs_iconv(EC_CHARSET, $GLOBALS['_SCFG']['mail_charset'], $content);
        $shop_name = ecs_iconv(EC_CHARSET, $GLOBALS['_SCFG']['mail_charset'], $GLOBALS['_SCFG']['shop_name']);
    }
    $charset   = $GLOBALS['_SCFG']['mail_charset'];
    /**
     * 使用mail函数发送邮件
     */
    if ($GLOBALS['_SCFG']['mail_service'] == 0 && function_exists('mail'))
    {
        /* 邮件的头部信息 */
        $content_type = ($type == 0) ? 'Content-Type: text/plain; charset=' . $charset : 'Content-Type: text/html; charset=' . $charset;
        $headers = array();
        $headers[] = 'From: "' . '=?' . $charset . '?B?' . base64_encode($shop_name) . '?='.'" <' . $GLOBALS['_SCFG']['smtp_mail'] . '>';
        $headers[] = $content_type . '; format=flowed';
        if ($notification)
        {
            $headers[] = 'Disposition-Notification-To: ' . '=?' . $charset . '?B?' . base64_encode($shop_name) . '?='.'" <' . $GLOBALS['_SCFG']['smtp_mail'] . '>';
        }
    ...
    else
    {
        ...
        /* 获得邮件服务器的参数设置 */
        $params['host'] = $GLOBALS['_SCFG']['smtp_host'];
        $params['port'] = $GLOBALS['_SCFG']['smtp_port'];
        $params['user'] = $GLOBALS['_SCFG']['smtp_user'];
        $params['pass'] = $GLOBALS['_SCFG']['smtp_pass'];
        ...
        else
        {
            ...
            $send_params['from']       = $GLOBALS['_SCFG']['smtp_mail'];
