...
function success(dom) { $(dom).attr("class", "").next("p").attr("class", "").html('<img src="http://www.transportjp.com/templates/default/images/gou.gif" />'); }
...
var citynamechecked = false;
function cityBlur(dom) {
    var cityname = $.trim($(dom).val());
    if (cityname.length <= 0) { $(dom).attr("class", "").val("").next("p").attr("class", "").text("输入不能为空"); citynamechecked = false; }
    else {
		if (true) {
            var citynamelength = 0;
            var d = cityname.toLowerCase(); 
            if (true) {
                $.ajax({
                    type: "POST",
                    url: "/ajax/user_ajax.php?action=checkuname",
                    ...
                    data: {username:d},
                    timeout: 10000,
                    error: function() { citynamechecked = true; $(dom).attr("class", "").next("p").attr("class", ""); },
                    success: function(msg) {
                        if (msg="OK") {
                            success(dom);
                            citynamechecked = true;
                        }
                    }
                });
            }
