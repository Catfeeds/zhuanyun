 ...
 * UTSOFT 表单验证类
 * ============================================================================
 * 版权所有 (C) 2005-2011 青岛友腾信息技术有限公司，并保留所有权利。
 * 网站地址 : http://qdutsoft.com
 * ----------------------------------------------------------------------------
 * ============================================================================
 * $Author : zhangxingpneg $
...
var Validator = function(name)
{
  ...
  * @param :  msg         错误提示信息
  */
  this.selectRequired = function(controlId,msg)
  {
    var obj = document.forms[this.formName].elements[controlId];
    if (typeof(obj) == "undefined" || Utils.trim(obj.value) == "0")
    {
      this.addErrorMsg(msg);
    }
  }
  ...
  * @param :  msg         错误提示信息
  */
  this.checkboxesRequired = function(controlName,msg)
  {
	var checkboxes = document.getElementsByName(controlName);
  ...
  * @param :  required    是否必须
  */
  this.isPostalCode = function(controlId, msg, required)
  {
    var obj = document.forms[this.formName].elements[controlId];
    obj.value = Utils.trim(obj.value);
    ...
    if ( ! Utils.isPostalCode(obj.value))
    {
      this.addErrorMsg(msg);
    }
  }
  ...
  */
  this.eqaul = function(fstControl, sndControl, msg)
  {
    ...
    {
      if (fstObj.value != sndObj.value)//fstObj.value == '' || 
      {
        this.addErrorMsg(msg);
      }
    }
  ...
  *
  * @param : fstControl   表单元素的ID
  * @param : minValue   应大于的值
  * @param : msg                错误提示信息
  */
  this.gtInt = function(fstControl, minValue, msg)
  {
    var fstObj = document.forms[this.formName].elements[fstControl];

    if (fstObj != null) {
      if (Utils.isNumber(fstObj.value) && Utils.isNumber(minValue)) {
        var v1 = parseFloat(fstObj.value) + 0;
        var v2 = parseFloat(minValue) + 0;
      } else {
        var v1 = fstObj.value;
        var v2 = minValue;
      }
  ...
  */
   ...
   * 判断用户金额是否能够支付
   */
 this.isPayMoney = function(usermoney, paymoney, msg)
 {
	  if (parseFloat(usermoney) > parseFloat(paymoney))
	  {
	       return;
	  }
	  else
